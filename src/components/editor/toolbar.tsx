...
const FontSizeButton = () => {
  ...
  const increment = () => {
    ...
    if (newSize > 0) {
      updateFontSize(newSize.toString().trim());
    }
  };
    ...
    <div className="flex items-center gap-x-0.5">
      ...
      {isEditing ? (
        <input
          ...
          className={cn(
            'h-7 w-10 text-sm text-center shrink-0 border border-neutral-400 rounded-sm bg-transparent focus:outline-none focus:ring-0',
          )}
        />
      ) : (
        <button
          ...
          className={cn(
            'h-7 w-10 text-sm text-center shrink-0 border border-neutral-400 rounded-sm bg-transparent cursor-text hover:bg-neutral-200/80',
          )}
        >
          {currentFontSize}
        </button>
...
const HeadingLevelButton = () => {
  ...
  const headings = [
    { label: 'Normal text', value: 0, fonSize: '16px' },
